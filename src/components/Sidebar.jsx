...
import { LayoutGrid, Users, Users2, Wallet, Calendar, AlertCircle, FileText, MessageCircle, BarChart3, Trophy, History, Settings, LogOut, Bell, } from 'lucide-react';
...
export function Sidebar({ activePage, onNavigate, user, onLogout, className }) {
    ...
    const initials = user?.name
        ...
        : '??';
    return (<aside className={cn('w-[240px] bg-sidebar text-sidebar-foreground', 'border-r border-sidebar-border', 'h-[100dvh] flex flex-col z-20', className)}>
      {/* ── Brand ── */}
      <div className="px-5 pt-[22px] pb-5">
        <button type="button" onClick={() => onNavigate('dashboard')} className="flex items-center gap-3 group focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40 rounded-xl p-1 -m-1" aria-label="Go to dashboard">
          <div className="w-[34px] h-[34px] rounded-xl border border-border bg-card flex items-center justify-center flex-shrink-0 shadow-sm overflow-hidden">
            <img src="/logo512.png" alt="Excellent Susu" className="w-[28px] h-[28px] object-contain"/>
          </div>
      ...
      <nav aria-label="Main navigation" className="flex-1 overflow-y-auto px-3 pb-4 no-scrollbar space-y-5">
        {sections.map((section) => (<div key={section.label}>
            <p className="px-3 mb-1.5 eyebrow text-sidebar-foreground/55 select-none">
              {section.label}
            </p>
            ...
            <div className="space-y-px">
              {section.items.map((item) => {
                ...
                return (<button key={item.id} type="button" onClick={() => onNavigate(item.id)} aria-current={isActive ? 'page' : undefined} className={cn('w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-base font-medium', 'transition-all duration-150', 'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40', 'relative', isActive
                        ? 'bg-primary/[0.08] text-primary'
                        : 'text-sidebar-foreground/80 hover:bg-sidebar-accent hover:text-sidebar-foreground/85')}>
                    {/* Left accent bar for active */}
                    {isActive && (<span className="absolute left-0 inset-y-[6px] w-[3px] rounded-r-full bg-primary" aria-hidden="true"/>)}
                    <Icon className={cn('w-4 h-4 flex-shrink-0', isActive ? 'text-primary' : 'text-sidebar-foreground/60')} strokeWidth={isActive ? 2.25 : 1.75}/>
      ...
      <div className="px-3 pb-4 pt-3 border-t border-sidebar-border">
        ...
        <div className="flex items-center gap-2.5 px-3 py-2.5 rounded-xl mb-px">
          ...
          <div className="flex-1 min-w-0 leading-none">
            <p className="app-row-title truncate text-sidebar-foreground">
              {user?.name}
            </p>
            <p className="eyebrow text-sidebar-foreground/60 mt-0.5 truncate">
              {user?.role}
            </p>
          </div>
        </div>
        ...
        {/* Sign out */}
        <button type="button" onClick={onLogout} className={cn('w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-base font-medium', 'text-sidebar-foreground/60', 'hover:bg-destructive/[0.06] hover:text-destructive', 'transition-all duration-150', 'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-destructive/40')}>
